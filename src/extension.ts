...
async function inferPreferredLanguage(
  diff: string,
  apiKey: string
): Promise<string> {
  const body = {
    model:
      vscode.workspace.getConfiguration().get<string>("aiConvCommit.model") ||
  ...
  if (["es", "en", "fr", "de"].includes(detected)) {
    return detected;
  }
  return "en";
}
async function generateCommitMessage(body: any, apiKey: string): Promise<void> {
  try {
    const response = await fetch(
      "https://openrouter.ai/api/v1/chat/completions",
      {
    ...
    if (!msg) {
      vscode.window.showErrorMessage("No se pudo generar el mensaje");
      return;
    }
...
export function activate(context: vscode.ExtensionContext) {
  let disposable = vscode.commands.registerCommand(
    "aiConvCommit.generate",
    async () => {
      const apiKey = vscode.workspace
        .getConfiguration()
        .get<string>("aiConvCommit.apiKey");
      if (!apiKey) {
      ...
      exec("git diff --cached", { cwd: gitRoot }, async (err, stdout) => {
        ...
        let lang =
          vscode.workspace
            .getConfiguration()
            .get<string>("aiConvCommit.language") || "auto";
        ...
        await vscode.window.withProgress(
          {
            ...
            title: "Generando mensaje de comit...",
            cancellable: false,
          },
          async (progress) => {
			progress.report({ increment: 0 });
			await generateCommitMessage(body, apiKey);
			progress.report({ increment: 100 });
			vscode.window.showInformationMessage(
			  "Mensaje de commit generado con éxito."
			);
		  }
        );
      });
  ...
  if (repo) {
    ...
    repo.inputBox.placeholder =
      "Haz clic en ✨ para generar mensaje automáticamente";
  }
}
